...
VERBOSE_OUT=${VERBOSE_OUT:-/dev/null}
...
build-localnet() {
  ...
  docker compose -f localnet/docker-compose.yml -p "chainflip-localnet" up $INITIAL_CONTAINERS -d $additional_docker_compose_up_args > "$VERBOSE_OUT" 2>&1

  echo "🏗 Building network"
  docker compose -f localnet/docker-compose.yml -p "chainflip-localnet" up $CORE_CONTAINERS -d $additional_docker_compose_up_args > "$VERBOSE_OUT" 2>&1

  echo "🪙 Waiting for Bitcoin node to start"
  check_endpoint_health -s --user flip:flip -H 'Content-Type: text/plain;' --data '{"jsonrpc":"1.0", "id": "1", "method": "getblockchaininfo", "params" : []}' http://localhost:8332 > "$VERBOSE_OUT"

  echo "💎 Waiting for ETH node to start"
  check_endpoint_health -s -H "Content-Type: application/json" --data '{"jsonrpc":"2.0","method":"net_version","params":[],"id":67}' http://localhost:8545 > "$VERBOSE_OUT"
  wscat -c ws://127.0.0.1:8546 -x '{"jsonrpc":"2.0","method":"net_version","params":[],"id":67}' > "$VERBOSE_OUT"
  ...
  docker compose -f localnet/docker-compose.yml -p "chainflip-localnet" up $ARB_CONTAINERS -d $additional_docker_compose_up_args > "$VERBOSE_OUT" 2>&1
  ...
  RPC_PORT=$INIT_RPC_PORT
  for NODE in "${SELECTED_NODES[@]}"; do
    check_endpoint_health -s -H "Content-Type: application/json" -d '{"id":1, "jsonrpc":"2.0", "method": "chain_getBlock"}' "http://localhost:$RPC_PORT" > "$VERBOSE_OUT"
...
destroy() {
  ...
  docker compose -f localnet/docker-compose.yml -p "chainflip-localnet" down $additional_docker_compose_down_args > "$VERBOSE_OUT" 2>&1
...
bouncer() {
  (
    cd ./bouncer
    pnpm install > "$VERBOSE_OUT" 2>&1
    ./run.sh
  )
}

main() {
    if ! which wscat > "$VERBOSE_OUT"; then
