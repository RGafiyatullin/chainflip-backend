...
use cf_traits::{
	ActiveValidatorRange, AuctionError, AuctionIndex, AuctionResult, Auctioneer,
...
pub mod pallet {
	use super::*;
	use cf_traits::{
		AuctionIndex, AuctionResult, Chainflip, ChainflipAccount, EmergencyRotation, EpochInfo,
...
impl<T: Config> Auctioneer for Pallet<T> {
	...
	fn run_auction() -> Result<AuctionResult<Self::ValidatorId, Self::Amount>, AuctionError> {
		...
		if T::EmergencyRotation::emergency_rotation_in_progress() {
			...
			if let Some(AuctionResult { minimum_active_bid, .. }) = LastAuctionResult::<T>::get() {
				if let Some(new_target_validator_group_size) =
					next_validator_group.iter().position(|(_, amount)| amount < &minimum_active_bid)
				{
		...
		return Ok(AuctionResult { winners: validating_set.clone(), minimum_active_bid })
	}
	...
	fn confirm_auction(auction: AuctionResult<Self::ValidatorId, Self::Amount>) {
		...
		// Store the result
		LastAuctionResult::<T>::put(auction);
