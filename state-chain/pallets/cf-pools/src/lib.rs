...
impl<T: Config> Pallet<T> {
	fn inner_sweep(
		...
		/*for canonical_asset_pair in Pools::<T>::iter_keys().collect::<Vec<_>>() {
			...
			for (side, limit_orders_cache) in pool.limit_orders_cache.as_ref().into_iter().filter_map(|(side, limit_orders_cache)| {
				limit_orders_cache.get(lp).cloned().map(|limit_orders_cache| (side, limit_orders_cache))
			}).collect::<Vec<_>>() {
				let asset_pair = AssetPair {
					canonical_asset_pair,
					base_side: side,
