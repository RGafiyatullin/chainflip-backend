...
impl<T: Config> CfeMultisigRequest<T, SolanaCrypto> for Pallet<T> {
	fn keygen_request(_req: cfe_events::KeygenRequest<<T as Chainflip>::ValidatorId>) {
		unimplemented!()
	}

	fn signature_request(
		_req: cfe_events::ThresholdSignatureRequest<<T as Chainflip>::ValidatorId, SolanaCrypto>,
	) {
		unimplemented!()
	}
}
...
impl<T: Config> CfeBroadcastRequest<T, Solana> for Pallet<T> {
	fn tx_broadcast_request(
		_req: cfe_events::TxBroadcastRequest<<T as Chainflip>::ValidatorId, Solana>,
	) {
		unimplemented!()
	}
}
